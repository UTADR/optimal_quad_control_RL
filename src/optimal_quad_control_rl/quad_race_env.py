...
start_pos = gate_pos[0] + np.array([0, -1.0, 0])
...
class Quadcopter3DGates(VecEnv):
    def __init__(
        ...
        def rand_f(n):
            ...
            return np.array([param_dict[p] for p in params]).T
        ...
        if self.param_input:
            params_with_noise = self.params * np.random.uniform(
                1 - self.param_input_noise,
                1 + self.param_input_noise,
                size=self.params.shape,
            )
            self.param_encoding = quad_model.param_encoding(params_with_noise)
        ...
        self.state_hist = np.zeros(
            (num_envs, num_hist, self.state_len), dtype=np.float32
        )
    ...
    def update_states_gate(self):
        ...
        for i in range(self.gates_ahead):
            ...
            new_states[valid, 16 + 4 * i : 16 + 4 * i + 3] = self.gate_pos_rel[
                indices[valid]
            ]
        ...
        if self.param_input:
            new_states[:, 16 + 4 * self.gates_ahead + 4 * self.num_action_history :] = (
                self.param_encoding
            )
        ...
        self.states = self.state_hist[
            :,
            0 : (self.num_state_history + 1)
            * self.history_step_size : self.history_step_size,
    ...
    def reset_(self, dones):
        ...
        if self.initialize_at_random_gates:
            ...
            self.target_gates[dones] = np.random.randint(
                0, self.num_gates, size=num_reset
            )
        ...
        if self.param_input:
            params_with_noise = self.params[dones] * np.random.uniform(
                1 - self.param_input_noise,
                1 + self.param_input_noise,
                size=self.params[dones].shape,
            )
            self.param_encoding[dones] = quad_model.param_encoding(params_with_noise)
    ...
    def step_wait(self):
        ...
        action_penalty_delta = 0.001 * np.linalg.norm(
            (self.actions - self.prev_actions), axis=1
        )
        ...
        gate_collision = passed_gate_plane & np.any(
            np.abs(pos_new - pos_gate) > gate_size / 2, axis=1
        )
        ...
        out_of_bounds = np.any(
            np.abs(new_states[:, 0:2]) > 5, axis=1
        )  # edges of the grid
        out_of_bounds |= new_states[:, 2] < -7  # max height (z-axis point down)
        out_of_bounds |= np.any(
            np.abs(new_states[:, 9:12]) > 1000, axis=1
        ...
        dones = (
            max_steps_reached | ground_collision | out_of_bounds | gate_collision
        )  # | self.final_gate_passed
        self.dones = dones
