...
def randomization_fixed_params_5inch(num):
    return {key: np.repeat(value, num) for key, value in params_5inch.items()}
...
def randomization_5inch_10_percent(num):
    return {
        key: (
            np.random.uniform(value * 0.9, value * 1.1, num)
            if key != "k"
...
# 20% randomization
def randomization_5inch_20_percent(num):
    return {
        key: (
            np.random.uniform(value * 0.8, value * 1.2, num)
            if key != "k"
...
# 30% randomization
def randomization_5inch_30_percent(num):
    return {
        key: (
            np.random.uniform(value * 0.7, value * 1.3, num)
            if key != "k"
...
def randomization_fixed_params_3inch(num):
    return {key: np.repeat(value, num) for key, value in params_3inch.items()}


# 10% randomization - uniform distribution between 0.9 and 1.1 of the original value. However, for k we must ensure that it is between 0 and 1
def randomization_3inch_10_percent(num):
    return {
        key: (
            np.random.uniform(value * 0.9, value * 1.1, num)
            if key != "k"
...
# 20% randomization
def randomization_3inch_20_percent(num):
    return {
        key: (
            np.random.uniform(value * 0.8, value * 1.2, num)
            if key != "k"
...
# 30% randomization
def randomization_3inch_30_percent(num):
    return {
        key: (
            np.random.uniform(value * 0.7, value * 1.3, num)
            if key != "k"
